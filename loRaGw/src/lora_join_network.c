...
LORA_Join_Network_Status_t loRaJoinPacketHandler(uint8_t message_type, uint8_t *payload, uint16_t payload_len)
{
    ...
    if (message_type == LORA_JOIN_TYPE_CONFIRM){
        return loRaJoinNetworkGetConfirm(payload, payload_len, &join_info);
    }
...
// === JOIN COMPLETE IMPLEMENTATION ===

/**
 * @brief Create Join Complete
 */
LORA_Join_Network_Status_t loRaCreateJoinComplete(const uint8_t *mac_address, const uint8_t *unicast,
                                                uint8_t *frame_buffer, uint16_t *frame_length)
{
    if (!mac_address || !unicast || !frame_buffer || !frame_length)  {
        return LORA_JOIN_STATUS_NULL_POINTER;
    }
    ...
    // Validate unicast 
    static const uint8_t unicast_zero[2] = {0};
    if (memcmp(unicast, unicast_zero, 2) == 0 ) {
        return LORA_JOIN_STATUS_INVALID_KEY;
    }

    uint8_t payload[6 + 2];
    memcpy(payload, mac_address, 6);
    memcpy(payload + 6, unicast, 2);

    // Đóng gói frame
    LORA_frame_t lora_frame = {
        .packet_type   = LORA_PACKET_TYPE_JOIN,
        .message_type  = LORA_JOIN_TYPE_COMPLETED,
        .data_len      = 6 + 2,
        .data_payload  = payload
    };
    ...
    *frame_length = LORA_FRAME_HEADER_SIZE + lora_frame.data_len + sizeof(crc_t) + 1; // 1 = encrypted flag byte
    return LORA_JOIN_STATUS_CREATE_COMPLETE_SUCCESS;
}
                                                ...
                                                uint8_t *frame_buffer, uint16_t *frame_length)
{
    ...
    LORA_frame_t lora_frame = {
        .packet_type   = LORA_PACKET_TYPE_JOIN,
        .message_type  = LORA_JOIN_TYPE_CONFIRM,
        .data_len      = 6 + 16,
        .data_payload  = payload
    };
