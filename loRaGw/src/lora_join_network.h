...
typedef enum
{
    ...
    LORA_JOIN_STATUS_CREATE_COMPLETE_SUCCESS = 9,
    LORA_JOIN_STATUS_GET_COMPLETE_SUCCESS = 10
...
//     bool has_joined;
//     uint32_t last_seen;
//     uint8_t join_state; // 0=request, 1=accepted, 2=confirmed, 3=complete
// } NodeInfo;
...
// GatewayState gateway = {.next_unicast_addr = 1000}; // bắt đầu từ địa chỉ 1000
...
// Gateway side: join Complete
LORA_Join_Network_Status_t loRaCreateJoinComplete(const uint8_t *mac_address, const uint8_t *unicast, uint8_t *frame_buffer, uint16_t *frame_length);
