...
void test_crc_fast_frame_data(void) 
{ 
    
    uint8_t data[] = {0xAA, 0x02, 0x00, 0x00, 0x01, 0x00}; 
    int data_length = sizeof(data) / sizeof(data[0]); 
    crc_t expected = 0x6A9EF10A; // Giá trị CRC-32 đã biết cho "12345" 
    crc_t result = crc_fast(data, data_length); 
    
    TEST_ASSERT_EQUAL_HEX32(expected, result); 
}

void test_crc_slow_frame_data(void) 
{ 
    uint8_t data[] = {0xAA, 0x02, 0x00, 0x00, 0x01, 0x00}; 
    int data_length = sizeof(data) / sizeof(data[0]); 
    crc_t expected = 0x6A9EF10A; // Giá trị CRC-32 đã biết cho "12345" 
