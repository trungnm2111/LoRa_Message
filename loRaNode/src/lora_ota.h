...
typedef enum {
    ...
    OTA_TYPE_END             = 0x01,    /* payload = 0x01 */
    OTA_TYPE_SIGNAL_UPDATE   = 0x02,    /* payload = 0x02 */
    OTA_TYPE_RESPONSE        = 0x03,    /* payload = 0x00(ACK)/0x01(NACK) */
    OTA_TYPE_HEADER          = 0x04,    /* payload = 16 bytes (ota_header_t) */
    OTA_TYPE_DATA            = 0x05     /* payload = seq(2B) + data(nB) */
} ota_message_type_t;
